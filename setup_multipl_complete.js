...
async function setupMultiplComplete() {
  console.log('🚀 Starting complete Multipl setup...\n');
  
  try {
    // 1. Create Multipl user
    console.log('1️⃣ Creating Multipl user...');
    const multiplUser = {
    ...
    console.log(`✅ Multipl user created with ID: ${multiplUserId}\n`);
    ...
    const answers = [
      {
        ...
        answerText: 'Our TAM is the Indian savings and investment market, which is $500B+ and growing at 15% annually. We focus on the goal-based savings segment ($50B TAM) where consumers save for specific purchases. We plan to capture this through our embedded finance model with partner brands and our disciplined savings platform that increases customer conversions for brands while providing users a structured way to save for discretionary purchases.'
      },
      {
        questionId: 'q2',
        questionText: 'What is your customer acquisition cost (CAC) and customer lifetime value (LTV)?',
        answerText: 'Our CAC is ₹150 through our partner brand integrations and digital marketing. Our LTV is ₹2,500 based on average user engagement of 18 months and our revenue model. We achieve a healthy LTV:CAC ratio of 16:1, which is sustainable for our business model. Our unit economics are strong with positive contribution margins from day one.'
      },
      {
        questionId: 'q3',
        questionText: 'What is your monthly recurring revenue (MRR) and growth trajectory?',
        answerText: 'Our current MRR is ₹4.2 Cr with 150% year-over-year growth. We have 700K+ registered users and 200K+ KYC-verified accounts with INR 100+ Cr Assets Under Advice. We project CY26 revenue of INR 50 Cr+ and are growing at 15% month-over-month. Our active goals are 100K+ with target goals set at INR 600+ Cr.'
      },
      {
        questionId: 'q4',
        questionText: 'Who are your main competitors and how do you differentiate?',
        answerText: 'Our main competitors are traditional savings apps like PiggyVest and investment platforms like Groww. We differentiate through our embedded finance model with partner brands - we\'re not just a savings app, we\'re a conversion tool for brands. Our unique value proposition is enabling consumers to save towards specific spending goals with partner brands, creating a win-win for both users and brands.'
      },
      {
        questionId: 'q5',
        questionText: 'What is your go-to-market strategy and customer acquisition approach?',
        answerText: 'Our go-to-market strategy is B2B2C through partner brand integrations. We acquire customers through our partner brands in categories like travel, shopping, and gadgets. Our approach is to embed our savings functionality into the brand\'s customer journey, increasing conversions for brands while providing users a disciplined savings vehicle. We also use digital marketing and referral programs to drive organic growth.'
      }
    ];
    ...
    await userRef.update({
      ...
      description: dealNoteData.brief?.brief_1_2_sentences || 'Multipl is an Indian fintech platform that enables consumers to save and invest towards specific spending goals with partner brands.',
    ...
    fs.writeFileSync('multipl_complete_setup.json', JSON.stringify(setupData, null, 2));
    console.log('📄 Setup data saved to multipl_complete_setup.json');
    
    console.log('\n🎉 Complete Multipl setup finished successfully!');
    console.log('\n📊 Setup Summary:');
    ...
    console.error('❌ Error in Multipl setup:', error);
    process.exit(1);
  }
}

setupMultiplComplete();
